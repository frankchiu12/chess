...
public class Pawn extends ChessPiece {
    ...
    public void overallMovement(ArrayList<Coordinate<Integer, Integer>> coordinateArrayList) {
        ...
        if (this.game.checkCanMove(this.getRow(), this.getColumn(), - 1, 0) && this.color == this.game.getPlayerColor()){
            coordinateArrayList.add(new Coordinate<>(this.getRow() - 1, this.getColumn()));
        }

        // eat right diagonal
        if (this.getRow() > 0 && this.getColumn() < 7 && this.game.checkCanEat(this.getRow(), this.getColumn(), -1, 1, this.color) && this.color == this.game.getPlayerColor()){
            coordinateArrayList.add(new Coordinate<>(this.getRow() - 1, this.getColumn() + 1));
        }

        // eat left diagonal
        if (this.getRow() > 0 && this.getColumn() > 0 && this.game.checkCanEat(this.getRow(), this.getColumn(), -1, -1, this.color) && this.color == this.game.getPlayerColor()){
            coordinateArrayList.add(new Coordinate<>(this.getRow() - 1, this.getColumn() - 1));
        }
